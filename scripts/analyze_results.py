...
Analyzes all 166 experiments to answer research questions
"""
...
def parse_disease(exp_name: str) -> str:
    """Extract disease from experiment name"""
    ...
    # IBD is default for experiments without prefix
...
def parse_model_type(exp_name: str) -> str:
    ...
    # Check CNN first to avoid confusion with cnn_baseline
    if 'cnn' in exp_name:
        return 'CNN'
...
def compare_two_experiments(df: pd.DataFrame, exp1: str, exp2: str, 
    ...
    row1 = df[df['experiment'] == exp1].iloc[0]
    row2 = df[df['experiment'] == exp2].iloc[0]
    ...
    # Approximate t-test (assumes normal distribution)
    n1 = row1['num_runs']
    n2 = row2['num_runs']
...
def analyze_research_questions(df: pd.DataFrame) -> str:
    ...
    # Q1: Do evolutionary relationships help?
    ...
    for disease in df['disease'].unique():
        ...
        # Find GINEConv baseline and MLP
    ...
    for disease in df['disease'].unique():
        ...
        if not binary_exp.empty and not age_exp.empty:
            ...
            diff = binary_acc - age_acc
            
            report.append(f"  {disease.upper():20s}: Binary={binary_acc:.3f} vs Age={age_acc:.3f} | Δ={diff:+.3f}")
...
def main():
    ...
    # Load results
    print("Loading results from 166 experiments...")
    df = load_all_results()
