...
def load_data():
    ...
    df = pd.read_csv(ANALYSIS_DIR / "all_results.csv")
    return df

def plot_gnn_vs_mlp(df):
    """Compare GNN vs MLP across diseases"""
    ...
    ax.set_title('Q1: GNN vs MLP - Les graphes phylogénétiques aident-ils?')
    ax.set_xticks(x)
    ax.set_xticklabels(diseases, rotation=45, ha='right')
    ax.legend()
    ax.axhline(y=0.5, color='r', linestyle='--', alpha=0.3, label='Hasard')
    ax.set_ylim(0.5, 0.7)
...
def plot_gnn_vs_cnn(df):
    """Compare GNN vs CNN across diseases"""
    fig, ax = plt.subplots(figsize=(14, 8))
    ...
    ax.bar([i - width/2 for i in x], gnn_acc, width, label='GNN (graphes)', alpha=0.8)
    ax.bar([i + width/2 for i in x], cnn_acc, width, label='CNN (voisinage fixe)', alpha=0.8)
    
    ax.set_xlabel('Maladie')
    ax.set_ylabel('Accuracy')
    ax.set_title('Q2: GNN vs CNN - Les graphes sont-ils plus flexibles?')
    ax.set_xticks(x)
    ax.set_xticklabels(diseases, rotation=45, ha='right')
    ax.legend()
    ax.axhline(y=0.5, color='r', linestyle='--', alpha=0.3, label='Hasard')
...
def plot_binary_vs_multiclass(df):
    ...
    ax.bar([i - width/2 for i in x], binary_acc, width, label='Binaire (maladie)', alpha=0.8)
    ax.bar([i + width/2 for i in x], age_acc, width, label='Multi-classe (âge)', alpha=0.8)
    
    ax.set_xlabel('Maladie')
    ax.set_ylabel('Accuracy')
    ax.set_title('Q3: Classification Binaire vs Multi-classes')
    ax.set_xticks(x)
    ax.set_xticklabels(diseases, rotation=45, ha='right')
    ax.legend()
    ax.axhline(y=0.5, color='r', linestyle='--', alpha=0.3, label='Hasard (binaire)')
    ax.axhline(y=0.2, color='orange', linestyle='--', alpha=0.3, label='Hasard (5 classes)')
    ax.set_ylim(0.5, 0.7)
...
def plot_edge_weights(df):
    """Compare edge weighting strategies"""
    ...
    ax.set_title('Q4: Comparaison des Stratégies de Pondération d\'Arêtes (IBD)')
    ax.axvline(x=0.6, color='r', linestyle='--', alpha=0.3, label='60%')
...
def plot_architectures(df):
    ...
    ax.set_xlabel('Accuracy')
    ax.set_title('Q5: Comparaison des Architectures (IBD)')
    ax.axvline(x=0.6, color='r', linestyle='--', alpha=0.3, label='60%')
    ax.set_xlim(0.5, 0.7)
    
    # Legend
    from matplotlib.patches import Patch
    legend_elements = [Patch(facecolor='C0', label='GNN'),
                      Patch(facecolor='C1', label='Non-graphe')]
...
def plot_summary_heatmap(df):
    """Create summary heatmap of all diseases × architectures"""
    # Prepare data
    diseases = []
    ...
    for disease in df['disease'].unique():
        ...
        for model in models:
            ...
            if not model_df.empty:
                row.append(model_df['test_accuracy_mean'].values[0])
            else:
                row.append(None)
        data.append(row)
    ...
    ax.set_title('Résumé: Toutes Maladies × Toutes Architectures')
