#!/usr/bin/env python3
"""
Interactive experiment runner for IBD GNN project
"""
...
def get_experiment_status(exp_name):
    ...
    completed = sum(1 for r in runs if (r / "metrics.json").exists())
...
def get_experiments_for_disease(disease, experiments):
    """Get all experiments for a disease"""
    results = []
...
def show_header():
    print("\n" + "=" * 50)
    print("       IBD GNN Experiment Runner")
    print("=" * 50)
...
def select_from_list(items, prompt, allow_all=True, allow_back=True):
    """Generic selection from a list - supports multiple selections"""
    print(f"\n{prompt}")
...
def run_experiments_menu(experiments):
    ...
    while True:
        ...
        # Select disease
        disease_display = [d.replace("_", " ").title() for d in DISEASES]
        ...
        # Parse type choices (can be multiple like "1,2,3")
            ...
            if type_choice == '0' or not selected_types:
                ...
                if "arch" in selected_types:
                    for suffix in CATEGORIES["arch"]:
                        ...
                        # Handle special case for IBD baseline
